...
async function getCandidatePaths(cwd: string): Promise<string[]> {
  ...
  const explicitPath = process.env.PI_SENTRY_CONFIG;
  if (explicitPath) {
  ...
  addUnique(configDirs, join(cwd, ".pi"));

  const home = homedir();
  if (home) {
    addUnique(configDirs, join(home, ".pi", "agent"));
  }
...
function addEnvOverrides(
  ...
  const tracesSampleRate = parseNumberEnv("PI_SENTRY_TRACES_SAMPLE_RATE");
  if (tracesSampleRate !== undefined) {
    withEnv.tracesSampleRate = tracesSampleRate;
  }

  const recordInputs = parseBooleanEnv("PI_SENTRY_RECORD_INPUTS");
  if (recordInputs !== undefined) {
    withEnv.recordInputs = recordInputs;
  }

  const recordOutputs = parseBooleanEnv("PI_SENTRY_RECORD_OUTPUTS");
